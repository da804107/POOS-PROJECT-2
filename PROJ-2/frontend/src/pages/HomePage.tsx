...
const HomePage: React.FC = () => {
  const [studySets, setStudySets] = useState<
    { id: string; name: string; isEditing: boolean }[]
  >([]);
    ...
    const handleLoad = async () => {
      ...
      try {
        ...
        const response = await fetch(
          "https://project.annetteisabrunette.xyz/api/loadsets",
          requestOptions
        );
    ...
    if (newSetName.trim()) {
      ...
      try {
        ...
        setStudySets((prevStudySets) => [
          ...prevStudySets,
          { id: newSet.userId, name: newSet.title, isEditing: false },
        ]);
        setNewSetName("");
        setIsAdding(false);
      } catch (error) {
  ...
  async function doAddSet(set: {
    ...
    try {
      const response = await fetch(
        "https://project.annetteisabrunette.xyz/api/addset",
        requestOptions
      );
  ...
  async function doDeleteSet(setTitle: string): Promise<void> {
    ...
    try {
      const response = await fetch(
        "https://project.annetteisabrunette.xyz/api/deleteset",
        requestOptions
      );
  ...
  async function doUpdateSet(setId: string, setTitle: string): Promise<void> {
    ...
    try {
      ...
      console.error("Error during update set");
      throw error;
    }
  }
